...
export { drawProgressBar, loadUIAssets, blankTexture, ScreenText, glyphs }
...
function drawProgressBar(uniformBuffer: GPUBuffer, uniformBufferBg: GPUBuffer, renderPass: GPURenderPassEncoder, val: number, max: number, rect: Rect, 
    ...
    fgColor: [number,number,number,number] = [1, 0, 0, 1])
{
    ...
    mat4.scale(transform, [rect.width*1.1, rect.height*1.1, 1], transform);
    drawSprite(uniformBufferBg, renderPass, transform, blankTexture, bgColor);

    var pct = val/max;
    pct = Math.max(pct, 0);
    mat4.translate(transform, [(-0.5*(1-pct)) * 0.95, 0, 0], transform);
    mat4.scale(transform, [pct*0.95, 0.65, 1], transform);
