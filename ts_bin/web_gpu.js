...
function updateWorld(deltaTime, b) {
    ...
    if (spawnNewEnemy) {
        var enemy = new Enemy(Math.random() * (2 * Width / Height) + -Width / Height, Math.random() * 2 + -1, 0, 0, 0, 0, { x: 0, y: 0, width: 0.2, height: 0.2 }, 1);
        entities.push(enemy);
    }
}
...
function drawSprite(renderPass, transform, texture) {
...
async function render() {
    ...
    if (!Number.isNaN(deltaTime))
        accTime += SpaceDown ? deltaTime * 0.25 : deltaTime;
    let thresh = 0.008;
...
async function main() {
    ...
    var enemy = new Enemy(0, 0, 0, 0, 0, 0, { x: 0, y: 0, width: 0.2, height: 0.2 }, 10);
    entities.push(enemy);
    requestAnimationFrame(render);
}
main();
